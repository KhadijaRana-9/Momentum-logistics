...
import { intParam, objectIdParam, stringParam } from '../_lib/params.js';
...
export default route({
  ...
  // PATCH /api/followups?id=X  (query param, not a path segment — keeps this
  // on the same function as GET/POST under Vercel's 12-function Hobby limit)
  PATCH: async (req, res) => {
    const session = await requirePermission(req, 'followups:manage');
